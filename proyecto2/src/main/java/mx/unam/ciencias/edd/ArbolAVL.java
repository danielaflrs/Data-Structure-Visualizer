...
public class ArbolAVL<T extends Comparable<T>>
	...
	rebalanceoAVL(verticeAVL(vertice.padre));
    }

    private void rebalanceoAVL(VerticeAVL vertice){
	VerticeAVL p,q;
	if(vertice == null)
	    return;
	...
	if(balance(vertice) == -2){
		...
		calcAltura(p);
		calcAltura(verticeAVL(p.padre));
	    }
	    super.giraDerecha(vertice);
	    calcAltura(vertice);	
	}
	this.rebalanceoAVL(verticeAVL(vertice.padre));
    }
    ...
    private void calcAltura(VerticeAVL vertice){
	if(vertice == null)
	    return;
